...
- [x] **Project save/load** as portable JSON (`serialize`/`restore` in state.js; image embedded as a data URL). Save/Load buttons download/read a `.coverforge.json` file, and work auto-saves to localStorage and restores on reload. A confirmed **New cover** action clears text/art/layers and replaces the autosave with a blank project. `restore` deep-merges so control bindings keep their object references.
...
- [x] **Hardcover mode**: paperback/hardcover binding toggle with case-laminate geometry — wrap 0.51″, safe 0.635″, hinge 0.4″, barcode (0.76″ from bottom, 0.25″ from the hinge), pages 75–550, all confirmed against KDP's hardcover guide (see `docs/KDP_SPEC.md`). Hinge guides added. Spine width is auto-estimated with a user override field for KDP's exact calculator value, plus a "verify + proof" warning since KDP doesn't publish per-page hardcover thickness.
